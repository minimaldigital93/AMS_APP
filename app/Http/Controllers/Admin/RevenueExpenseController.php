...
class RevenueExpenseController extends Controller
{
    ...
    /**
     * Builds the income write service. Callers must have already verified an
     * active fiscal period exists (the service treats period as non-null).
     */
    ...
    public function addTenantCharge(AddTenantChargeRequest $request)
    {
        ...
        // Charge type is constrained to utility types by the validator above,
        // so the service can safely create the Utilities row unconditionally.
        $period = $this->getActiveFiscalPeriod();
        if ($period) {
    ...
    public function removeTenantCharge($chargeId)
    {
        ...
        // Service handles paid-charge protection + Accounts cleanup.
        $removed = $period
    ...
    public function recordExpense()
    {
        ...
        $businessExpenses = BusinessExpense::with('attachments')
