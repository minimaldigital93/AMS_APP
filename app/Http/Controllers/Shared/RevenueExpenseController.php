 ...
 * and Supervisor panels. Panel-specific behaviour lives in the abstract hooks
 * below plus the trait hooks (fiscalPeriodsQuery / ledgerUserId /
 * khqrRoutePrefix). Every supervisor property-scoping guard in here no-ops for
 * admin/superadmin via ScopesToSupervisorProperties::seesWholeAccount().
 */
abstract class RevenueExpenseController extends Controller
{
    use HandlesKhqrCheckout;
    use HasFiscalPeriodScope;
    use ScopesToSupervisorProperties;

    /** Panel slug ('admin' or 'supervisor') — drives the view + route prefixes. */
    abstract protected function panel(): string;

    /**
     * Where to send the user when no fiscal period is open (admins can create
     * one; supervisors can only be warned).
     */
    abstract protected function missingPeriodRedirect(string $messageKey = 'messages.flash_fp_required'): RedirectResponse;

    /** Panel-specific guard for deleting an "other expense" ledger row. */
    abstract protected function authorizeOtherExpenseDelete(Accounts $expense): void;

    /**
     * Both panels render the same view set (resources/views/shared/
     * revenue_expense); $panel tells the view which layout to extend and
     * which panel's routes to link.
     */
    protected function viewName(string $view): string
    {
        return 'shared.revenue_expense.'.$view;
    }

    /** Add the panel discriminator every shared view needs. */
    protected function panelViewData(array $data = []): array
    {
        return $data + ['panel' => $this->panel()];
    }

    /** Render a shared revenue_expense view for this panel. */
     ...
     */
    protected function authorizeSupervisorPropertyRow(?int $propertyId): void
    {
    ...
    public function index()
    {
        ...
        if (! $activePeriod) {
            return $this->missingPeriodRedirect('messages.flash_fp_required_track');
        }
        ...
        return $this->panelView('index', $revenueExpenseData);
    }
    ...
    public function breakEvenPoint(Request $request)
    {
        ...
        return $this->panelView('break_even', $data);
    }
    ...
    public function recordExpense()
    {
        ...
        $businessExpenses = $this->scopeAccountsToProperty(
            BusinessExpense::with('attachments')->where('user_id', $this->ledgerUserId())
        )
    ...
    {
        $this->authorizeOtherExpenseDelete($expense);
    ...
    public function fixedExpenses()
    {
        ...
        return $this->panelView('fixed_expenses', compact('apartments'));
    }
    ...
    public function generateMonthlyBills()
    {
        ...
        return $this->panelView('generate_bills', compact(
    ...
    public function monthlyCalendar(Request $request)
    {
        ...
        return $this->panelView('monthly_calendar', compact(
    ...
    public function incomeStatement(Request $request)
    {
        ...
        $businessExpenses = $this->scopeAccountsToProperty(
            BusinessExpense::where('user_id', $this->ledgerUserId())
        )
            ->where('fiscal_period_id', $activePeriod->id)
