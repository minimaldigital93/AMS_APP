    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('supervisor.tenants.update', $tenant->id) }}" enctype="multipart/form-data" class="space-y-6"
            ...
            <div class="bg-white rounded-xl border border-slate-100 p-6">
                <h2 class="text-lg font-bold text-gray-900 mb-4 text-center">{{ __('messages.tenant_photo') }}</h2>
                <div class="flex items-start justify-center gap-6">
                    ...
                    @elseif($tenant->photo_path)
                        {{-- Click the photo (or the + badge) to replace it --}}
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" class="flex-1 px-6 py-2.5 bg-slate-800 text-white font-medium rounded-lg hover:bg-slate-700 transition text-center">{{ __('messages.update') }}</button>
            </div>
        </form>

        {{-- Delete forms live outside the update form above (forms can't nest); the
             buttons in the documents card target them by id. --}}
        @foreach($tenant->attachments as $doc)
            <form id="delete-document-{{ $doc->id }}" action="{{ route('supervisor.tenants.destroy_document', [$tenant, $doc]) }}" method="POST" class="hidden" data-confirm="{{ __('messages.remove_attachment_confirm') }}">
                @csrf @method('DELETE')
            </form>
        @endforeach
    </div>
