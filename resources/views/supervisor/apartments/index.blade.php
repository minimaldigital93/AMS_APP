...
@section('title', __('messages.apartment_management'))
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-4 py-3">
                                @php
                                    // Progress through the current rental month — Rentals::stayProgress()
                                    // is the one implementation (this used to derive its own cycle from
                                    // the tenant's move-in day and ignore the rent collection day, so
                                    // the same room read differently here and on the floor plan).
                                    $stayRental = $tenant ? $apartment->rentals()->where('tenant_id', $tenant->id)->latest()->first() : null;
...
document.addEventListener('DOMContentLoaded', function() {
    window.processLeaveClick = function(event, tenantId, tenantName) {
        ...
        window.confirmAction({
            ...
            if (ok) window.location.href = `{{ url('/supervisor/tenants') }}/${tenantId}/leave`;
        });
    };
