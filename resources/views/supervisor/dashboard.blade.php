...
<div class="max-w-6xl mx-auto space-y-8">
    ...
    <div class="flex flex-row items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2 shrink-0" x-data="{ showFloorPeek: false }">
            ...
            <a href="{{ route('supervisor.revenue_expense.record_income', $isFullPeriod ? [] : ['month' => $displayMonth->month, 'year' => $displayMonth->year]) }}"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl border border-slate-100 p-5 summary-card cursor-pointer" data-card="occupancy">
            ...
            <p class="text-[11px] text-slate-400 mt-2 space-y-0.5">
                ...
                {{-- Out of the rentable stock: excluded from the totals and rate above --}}
                <span class="flex justify-between"><span>{{ __('messages.maintenance_short') }}</span><span class="font-medium text-slate-500">{{ $stats['apartments']['maintenance'] }}</span></span>
                @endif
            </p>
            </div>
        </div>
    ...
    <div class="bg-yellow-50 border border-yellow-300 rounded-xl p-6">
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-yellow-800 mt-1">{{ __('messages.admin_must_open_period') }}</p>
            </div>
        </div>
    </div>
    @endif
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50">
                        ...
                        <td class="py-2.5 text-right">{{ money($period->opening_balance) }}</td>
                        <td class="py-2.5 text-right">{{ money($period->closing_balance) }}</td>
