<nav class="space-y-1" x-data="{ expandedSections: { 'Property': false, 'Tenant': false, 'Revenue': {{ request()->routeIs('admin.fiscalperiod.*') ? 'true' : 'false' }}, 'Settings': {{ request()->routeIs('admin.billing.*', 'admin.settings.*') ? 'true' : 'false' }} } }">
    ...
    <div class="mt-4">
        ...
        <div x-show="expandedSections['Property']" 
            ...
            <a href="{{ route('admin.vehicles.index') }}" class="submenu-item nav-link flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm {{ request()->routeIs('admin.vehicles.*') ? 'text-blue-700 active' : 'text-gray-700 hover:text-blue-700' }} transition-all sidebar-transition">
                <span class="nav-icon sidebar-transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
