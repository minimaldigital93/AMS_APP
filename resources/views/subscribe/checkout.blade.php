    ...
    <script>
        function khqrCheckout({ statusUrl, loginUrl, expiresAt }) {
            ...
            return {
                ...
                // identical, so a working check was indistinguishable from a
                // dead button.
                checking: false,
                lastCheckedAt: null,
                lastCheckedLabel: '',
                ...
                // "Checked 12s ago" — the one thing that makes a check that
                // found nothing distinguishable from a check that never ran.
                refreshCheckedLabel() {
                ...
                async poll() {
                    ...
                    try {
                        ...
                        // ONLY a poll that actually reached the gateway may
                        // clear the miss counter. A poll the server-side
                        // cooldown absorbed has learned nothing, and treating it
                        // as proof of health is why the stall warning could
                        // never fire: the real sequence is error, cooldown,
                        // cooldown, cooldown, error — so a counter reset by
                        // cooldowns never reaches two.
                        ...
                        this.miss(); /* network blip — keep polling */
                    } finally {
                        this.checking = false;
                    }
                },
