...
<div class="container mx-auto py-8 max-w-4xl">
    ...
    <div class="flex flex-wrap gap-2">
        @if($monthlyPeriod->canClose())
            <form method="POST" action="{{ route('admin.fiscalperiod.monthly-period.close', [$fiscalperiod->id, $monthlyPeriod->id]) }}" onsubmit="return confirm('Close {{ $monthlyPeriod->name }}?')">
                @csrf
                <button class="bg-amber-600 text-white px-4 py-2 rounded-lg hover:bg-amber-700 text-sm font-semibold">Close Month</button>
            </form>
        @endif
