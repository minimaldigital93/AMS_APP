...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Apartment</h1>
            <p class="text-gray-600 mt-1">Update apartment unit {{ $apartment->apartment_number }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <form method="POST" action="{{ route('admin.apartments.update', $apartment->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="4" 
                          ...
                          class="w-full px-4 py-2 border @error('description') border-red-500 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $apartment->description) }}</textarea>
                @error('description')
                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="mt-8 pt-8 border-t border-red-200">
            <div class="bg-red-50 rounded-lg p-6 border border-red-200">
                ...
                <p class="text-red-700 mb-4">Once you delete an apartment, there is no going back. Please be certain.</p>
