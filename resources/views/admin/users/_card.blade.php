...
@php
    ...
    $tenantRecord = $cardRole === 'tenant'
        ...
        : null;
    // Mirrors _row.blade.php: owner row / your own row / superadmin are locked.
...
<div class="user-card flex items-center gap-2 px-3 py-2.5" data-name="{{ strtolower($user->name) }}" data-phone="{{ strtolower($user->phone ?? '') }}" data-role="{{ strtolower($cardRole) }}">
    ...
    @if($rowLocked)
        <span class="flex-shrink-0 inline-flex items-center gap-1 text-xs font-medium text-gray-400" title="{{ $isSelf ? __('messages.your_own_account') : __('messages.account_owner') }}">
