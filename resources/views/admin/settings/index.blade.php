...
<div class="min-h-screen bg-gray-100 py-8">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        @php
            ...
            // stored in USD, so they display via money_input() and are converted
            // back by the controller.
            $priceKeys = \App\Http\Controllers\Admin\SettingsController::PRICE_KEYS;
            $multilineKeys = ['company_address', 'owner_address'];
        ...
        <!-- Expense Categories -->
        <div>
            <p class="px-4 mb-2 text-[13px] font-medium uppercase tracking-wide text-gray-500">{{ __('messages.expenses_word') }}</p>
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <a href="{{ route('admin.settings.expense_categories') }}" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition">
                    <svg class="flex-shrink-0 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    ...
                    <div class="min-w-0">
                        <p class="text-[15px] text-gray-900">{{ __('messages.expense_categories') }}</p>
                        <p class="text-[13px] text-gray-500">{{ __('messages.expense_categories_hint') }}</p>
                    </div>
                    <svg class="ml-auto flex-shrink-0 w-4 h-4 text-gray-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                    </svg>
        ...
        <form method="POST" action="{{ route('admin.settings.updateBatch') }}" class="space-y-8" enctype="multipart/form-data" x-data="logoUploader()">
            ...
            <!-- Scan-to-pay QR: printed under the tenant's unpaid bill -->
            <div x-data="qrUploader()">
                <p class="px-4 mb-2 text-[13px] font-medium uppercase tracking-wide text-gray-500">{{ __('messages.payment_qr_code') }}</p>
                <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                    <div class="flex items-center gap-4 px-4 py-4">
                        <!-- Preview -->
                        <div class="flex-shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="mt-2 text-[13px] text-gray-500">{{ __('messages.payment_qr_code_hint') }}</p>
                            <p class="mt-1 text-[13px] text-gray-400">{{ __('messages.payment_qr_code_upload_hint') }}</p>
            ...
            <div>
                ...
                <div class="bg-white rounded-xl shadow-sm overflow-hidden divide-y divide-gray-100"@if($category === 'system') x-data="{ currency: @js(settings('system_currency', 'USD')) }"@endif>
                    ...
                    @endphp
                    @php $multiline = in_array($key, $multilineKeys, true); @endphp
                    <div class="flex {{ $multiline ? 'items-start' : 'items-center' }} gap-3 px-4 py-3"@if($key === 'khr_exchange_rate') x-show="currency === 'KHR'" x-cloak @endif>
                        ...
                        <span class="relative ml-auto inline-flex items-center">
                            <select name="settings[{{ $key }}]" id="{{ $key }}" class="{{ $selectClasses }}">
                                ...
                                @foreach(['male', 'female', 'other'] as $g)
                                    <option value="{{ $g }}" {{ $currentValue === $g ? 'selected' : '' }}>{{ __('messages.'.$g) }}</option>
                                @endforeach
                            </select>
                            <svg class="pointer-events-none absolute right-0 w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $chevronIcon }}" />
                        ...
                        @elseif(in_array($key, $priceKeys, true))
                        ...
                        @elseif($key === 'utility_meter_auto_calc')
                        ...
                        <label class="ml-auto relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="settings[{{ $key }}]" id="{{ $key }}" value="1"
                                class="sr-only peer" {{ (string) $currentValue === '1' ? 'checked' : '' }}>
