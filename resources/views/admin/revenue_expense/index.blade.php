                        ...
                        <tr class="{{ !$apt['has_active_rental'] ? 'text-slate-300' : 'text-slate-700' }}" x-show="showAll || {{ $apt['has_active_rental'] ? 'true' : 'false' }}">
                            ...
                            <td class="px-4 py-2 text-center">
                                ...
                                <div class="flex items-center justify-center gap-1">
                                    @if($apt['tenant_id'])
                                    <a href="{{ route('admin.tenants.show', $apt['tenant_id']) }}" title="{{ __('messages.view_tenant') }}"
                                        class="p-1 rounded bg-sky-100 text-sky-600 hover:bg-sky-200 transition">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                    </a>
                                    @endif
                                    <a href="{{ route('admin.apartments.show', $apt['apartment_id']) }}" title="{{ __('messages.view_apartment') }}"
                            ...
                            <tr class="{{ $expense->is_active ? '' : 'opacity-50' }}">
                                ...
                                <td class="px-2 py-1.5 text-center">
                                    <div class="flex justify-center gap-1">
                                        ...
                                        <form action="{{ route('admin.revenue_expense.delete_fixed_expense', $expense) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('messages.remove_q') }}')">
                                            @csrf @method('DELETE')
