...
<div class="max-w-6xl mx-auto space-y-8" x-data="billingManager()">
    ...
    <div class="flex items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.revenue_expense.index') }}" class="inline-flex items-center justify-center h-10 w-10 bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition flex-shrink-0" title="{{ __('messages.back') }}">
...
function billingManager() {
    return {
        ...
        printBill(rentalId) {
            if (!rentalId) return;
            const url = '{{ route('admin.revenue_expense.print_bill', 'RENTAL_ID') }}'.replace('RENTAL_ID', rentalId);
        ...
        async removeViewCharge(chargeId, index) {
            ...
            try {
                const res = await fetch('{{ url('/admin/revenue-expense/remove-charge') }}/' + chargeId, {
                    method: 'DELETE',
        ...
        async clearAllCharges() {
            ...
            const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
            try {
                await fetch('{{ url('/admin/revenue-expense/clear-charges') }}/' + this.viewRentalId, {
                    method: 'DELETE',
                    headers: { 'X-CSRF-TOKEN': csrf, 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
                });
                window.location.reload();
