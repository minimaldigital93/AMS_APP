...
<div class="max-w-3xl mx-auto space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-400 text-sm mt-1">{{ __('messages.update_floor_units') }}</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.floors.update', $floor) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-slate-100">
            <div class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="description" value="{{ old('description', $floor->description) }}" placeholder="{{ __('messages.optional_description') }}"
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex gap-3">
                <button type="submit" class="flex-1 bg-slate-800 hover:bg-slate-700 text-white text-sm font-medium py-2.5 px-5 rounded-lg transition">{{ __('messages.update_floor') }}</button>
    ...
    <div class="bg-white rounded-xl border border-slate-100 p-6">
        ...
        <form method="POST" action="{{ route('admin.floors.update', $floor) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">{{ __('messages.status') }}</label>
                ...
                <div class="flex items-end">
                    <button type="submit" name="action" value="add_apartment" class="w-full bg-slate-800 hover:bg-slate-700 text-white text-sm font-medium py-2.5 px-4 rounded-lg transition">{{ __('messages.add_unit') }}</button>
                </div>
            </div>
        </form>
    </div>
