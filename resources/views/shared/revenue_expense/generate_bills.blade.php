...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8 flex items-center justify-between">
        ...
        <div class="flex gap-3">
            <a href="{{ route($panel.'.revenue_expense.fixed_expenses') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition text-sm" title="{{ __('messages.manage_apartment_costs') }}">
            ...
            <a href="{{ route($panel.'.revenue_expense.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition text-sm" title="{{ __('messages.back') }}">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden {{ $bill['has_unbilled'] ? '' : 'opacity-60' }}">
                ...
                <div class="p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        {{-- Vehicle parking has no fixed-expense template to tick: the bill
                             run derives it from the tenant's registered vehicles, so it
                             carries no form inputs and rides with the apartment checkbox. --}}
                        @php $fromVehicles = $expense['from_vehicles'] ?? false; @endphp
                        <div class="border rounded-lg p-3 {{ $expense['is_billed'] ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200' }}">
                            ...
                            <div>
                                @if($fromVehicles)
                                <p class="text-right text-sm font-semibold text-red-600">{{ money($expense['amount']) }}</p>
                                @else
                    ...
                    @else
                    <p class="text-sm text-gray-400 text-center py-2">{{ __('messages.no_apt_costs_assigned') }} <a href="{{ route($panel.'.revenue_expense.fixed_expenses') }}" class="text-blue-600 underline">{{ __('messages.add_some') }}</a></p>
                    @endif
                </div>
            </div>
            @endforeach
    ...
    <div class="bg-white rounded-lg shadow-md p-12 text-center">
        ...
        <div class="flex justify-center gap-4">
            <a href="{{ route($panel.'.revenue_expense.fixed_expenses') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">{{ __('messages.setup_apt_costs') }}</a>
            <a href="{{ $panel === 'admin' ? route('admin.floors.index') : route('supervisor.apartments.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition">{{ __('messages.manage_apartments') }}</a>
        </div>
    </div>
    @endif
</div>
