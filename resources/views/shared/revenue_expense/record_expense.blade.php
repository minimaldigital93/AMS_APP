    ...
    <div class="mb-6 flex items-center justify-center">
        <div class="inline-flex max-w-full items-center bg-white rounded-xl border border-slate-100 px-1.5 sm:px-2 py-1.5 gap-0.5 sm:gap-1">
            ...
            <a href="{{ route($panel.'.revenue_expense.record_expense', ['month' => $prevMonth['month'], 'year' => $prevMonth['year']]) }}"
               class="inline-flex shrink-0 items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-lg text-slate-500 hover:bg-slate-50 hover:text-sky-600 transition">
            ...
            <div class="px-2 sm:px-4 py-2 min-w-0 sm:min-w-[180px] text-center">
                ...
                <span class="text-base sm:text-lg text-slate-400 ml-1">{{ $selectedMonth->format('Y') }}</span>
                @if($isCurrentMonth)
                    <span class="ml-2 hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700">{{ __('messages.current') }}</span>
                @elseif($selectedMonth->isFuture())
                    <span class="ml-2 hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-sky-50 text-sky-700">{{ __('messages.upcoming') }}</span>
                @else
                    <span class="ml-2 hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-slate-50 text-slate-500">{{ __('messages.past') }}</span>
                @endif
            </div>
            @if($nextMonth)
            <a href="{{ route($panel.'.revenue_expense.record_expense', ['month' => $nextMonth['month'], 'year' => $nextMonth['year']]) }}"
               class="inline-flex shrink-0 items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-lg text-slate-500 hover:bg-slate-50 hover:text-sky-600 transition">
            ...
            @endphp
            @if($currentInPeriod)
            <a href="{{ route($panel.'.revenue_expense.record_expense', ['month' => $nowMonth, 'year' => $nowYear]) }}"
               class="ml-0.5 sm:ml-1 shrink-0 inline-flex items-center px-2 sm:px-3 py-2 text-sm font-medium text-sky-600 bg-sky-50 rounded-lg hover:bg-sky-100 transition" title="{{ __('messages.today') }}">
    ...
    <div x-show="showForm" x-cloak x-transition class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:items-center">
        ...
        <div class="relative bg-white rounded-xl border border-slate-100 w-full max-w-2xl my-auto overflow-hidden" @click.away="showForm = false">
            ...
            <div class="p-6">
                ...
                <form action="{{ route($panel.'.revenue_expense.store_business_expense') }}" method="POST" enctype="multipart/form-data"
            ...
            <table class="min-w-full">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50">
                        ...
                        <td class="px-4 py-3 text-center">
                            <form action="{{ route($panel.'.revenue_expense.delete_other_expense', $oe) }}" method="POST" data-confirm="{{ __('messages.remove_expense_confirm') }}">
                                @csrf @method('DELETE')
                    ...
                    <tr class="hover:bg-slate-50/50">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                @endforeach
                                <form action="{{ route($panel.'.revenue_expense.delete_business_expense', $be) }}" method="POST" data-confirm="{{ __('messages.remove_expense_confirm') }}">
                                    @csrf @method('DELETE')
        ...
        <div class="md:hidden divide-y divide-slate-50">
            ...
            @php
                $bizJson = [
                    ...
                    'recurring'  => false,
                    'attachments' => [],
                    'delete_url' => route($panel.'.revenue_expense.delete_other_expense', $oe),
                ];
            @endphp
            <div class="flex items-center gap-3 px-4 py-3 active:bg-slate-50 transition">
                <div class="min-w-0 flex-1">
            ...
            @php
                $bizJson = [
                    ...
                    'attachments' => $be->attachments->map(fn ($a) => [
                        ...
                        'delete_url' => route($panel.'.revenue_expense.delete_business_expense_attachment', [$be, $a]),
                    ])->all(),
                    'delete_url' => route($panel.'.revenue_expense.delete_business_expense', $be),
                ];
            @endphp
            <div class="flex items-center gap-3 px-4 py-3 active:bg-slate-50 transition">
                <div class="min-w-0 flex-1">
