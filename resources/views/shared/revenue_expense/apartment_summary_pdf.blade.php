...
<body>
    ...
    <div style="margin-bottom:12px; display:flex; gap:8px; align-items:center;">
        <form id="auto-gen-form" method="POST" action="{{ route($panel.'.revenue_expense.process_bills_auto') }}" style="margin:0;">
            @csrf
        ...
        <a href="{{ route($panel.'.revenue_expense.apartment_summary_pdf', ['start' => $start, 'end' => $end, 'summary_only' => $summaryOnly ?? false, 'whole' => $wholeNumbers ?? false]) }}" target="_blank" style="padding:8px 12px; background:#2563eb; color:#fff; border-radius:6px; text-decoration:none;">{{ __('messages.download_pdf') }}</a>
