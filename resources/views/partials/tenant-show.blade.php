    ...
    <div class="bg-white rounded-xl border border-slate-100 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <form method="POST" action="{{ route('admin.users.reset-password', $tenant->user) }}"
                      data-confirm="{{ __('messages.confirm_reset_tenant_password', ['name' => $tenant->name]) }}"
                      data-confirm-title="{{ __('messages.reset_password') }}"
                      data-confirm-ok="{{ __('messages.confirm_reset_password_ok') }}">
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 border border-slate-200 hover:border-slate-300 text-slate-700 text-sm font-medium rounded-lg transition">
                ...
                <p class="text-xs text-slate-400">{{ __('messages.reset_password_shown_once') }}</p>
            </div>
        </div>
    </div>
    @endif
        ...
        <div class="bg-white rounded-xl border border-slate-100 p-6">
            ...
            <div class="flex flex-wrap items-center gap-2" x-data="{ renewOpen: false }">
                <a href="{{ route('admin.contracts.view', $contractRental) }}"
                   title="{{ __('messages.preview') }} / {{ __('messages.print') }}" aria-label="{{ __('messages.preview') }} / {{ __('messages.print') }}"
                   class="inline-flex items-center justify-center p-2 text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                </a>
                <a href="{{ route('admin.contracts.download', $contractRental) }}"
                   title="{{ __('messages.download') }}" aria-label="{{ __('messages.download') }}"
                   class="inline-flex items-center justify-center p-2 text-slate-700 bg-slate-100 border border-slate-200 hover:bg-slate-200 rounded-lg transition">
                ...
                <button type="button" @click="renewOpen = true"
                        title="{{ __('messages.regenerate_or_renew') }}" aria-label="{{ __('messages.regenerate_or_renew') }}"
                        class="inline-flex items-center justify-center p-2 text-amber-700 bg-amber-50 border border-amber-200 hover:bg-amber-100 rounded-lg transition">
